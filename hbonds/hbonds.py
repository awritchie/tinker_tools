...
class Frame :
    def __init__(self, coords, waters, x2, cutoff) :
        self.x2 = x2
        self.Coords = []
        for water in waters :
            if np.linalg.norm(coords[water[0]] - self.x2) > cutoff : continue
            c = []
            for each in water : 
                    c.append(coords[each])
            self.Coords.append(c)
    def dist(self, x) : 
        assert len(x) == 3
        return min(np.linalg.norm(self.x2 - x[1]), np.linalg.norm(self.x2 - x[2])) 
    def fsort(self) : 
        self.Coords = sorted(self.Coords, key=self.dist)

def elapsed_time(wstart, cstart) : 
    wend = time.time()
    cend = time.clock()
    if (wend - wstart) > 60*60*1.5 : 
        print "Elapsed time: %.3f hr Wallclock, %.3f hr CPU"%((wend-wstart)/60/60, (cend-cstart)/60/60)
    elif (wend - wstart) > 60*1.5 :
        print "Elapsed time: %.3f min Wallclock, %.3f min CPU"%((wend-wstart)/60, (cend-cstart)/60)
    else :
        print "Elapsed time: %.3f s Wallclock, %.3f s CPU"%((wend-wstart), (cend-cstart))
...
parser.add_option("-s", "--stop", dest="stop", help="Maximum number of frame    s to look at.\tDefault: None", default=None)
...
for i in range(nframes) :
    ...
    x1 = f5["X/%i"%(i+1)][a1]
    x2 = f5["X/%i"%(i+1)][a2]
    c = Frame(f5["X/%i"%(i+1)], waters, x2, dist+2)
    c.fsort()
    for each in c.Coords : 
        v = c.dist(each)
        if v > dist : break
        v1 = np.linalg.norm(each[1] - x2)
        v2 = np.linalg.norm(each[2] - x2)
        if v1 <= dist or v2 <= dist :
            if v1 < v2 :
                geo = geometry(x1, x2, each[1], each[0])
                ...
                geo = geometry(x1, x2, each[2], each[0])
                if geo[0] >= a12h and geo[1] >= a2ho :
                    Rs.append(v2)
                    G12H.append(geo[0])
                    G2HO.append(geo[1])
    ...
    if fi - pf >= nframes/500. : 
        print "\n%.3f%% complete."%(fi/nframes*100.),
        elapsed_time(wstart, cstart)
        pf = fi
...
for fr, h, a1, a2 in zip(Fr, Rs, G12H, G2HO) : 
    ofile.write(" %10i %20.4f %20.4f %20.4f\n"%(fr, h, a1, a2))
ofile.close()
