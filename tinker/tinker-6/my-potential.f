...
c Andrew Added, start
c           include the implicit solvent models
            call ksolv
c Andrew Added, end
c
...
c
         ...
         em = em + e
         ep = ep + ei
         monopole = monopole + ck*rr1
         dipole = dipole -scd*rr3 - scu*rr3
         quadrupole = quadrupole + scq*rr5
      end do
c
...
c
      ...
      print *, 'ep :',ep
      print *, 'fi :',fi
      print *, 'pot:',pot,dipole,quadrupole,
...
c
      ...
      character*120 potfile
c Andrew added, START
      character*120 potentialfile, fieldfile
      character*7 ext
      integer ijpot,ijlext,ijzeroes, ifield, l
      real*8 debyes
      include 'potent.i'
      include 'units.i'
      include 'polar.i'
...
c Andrew added, START
c           write out a potential file to make life easy
            if (use_polar) then
               ijpot = freeunit()
               ijlext = 3
               ijzeroes = 0
               call numeral (ijzeroes,ext,ijlext)
               potentialfile = filename(1:leng)//'.'//ext(1:ijlext)//'v'
               call version (potentialfile,'new')
               open (unit=ijpot,file=potentialfile,status='new')
               write(ijpot,400) n,title(1:ltitle)
  400          format (i6,2x,a)
               do i = 1, npgrid(j)
                  xi = pgrid(1,i,j)
                  yi = pgrid(2,i,j)
                  zi = pgrid(3,i,j)
                  if (dotarget) then
c                    units are Kcal/mol/e
                     write(ijpot,401) i,xi,yi,zi,epot(1,i,j),epot(2,i,j)
  401                format (i8,3x,3f12.6,2x,2f12.4)
                  end if
               end do
               close (unit=ijpot)
               write (iout,402) potentialfile(1:trimtext(potentialfile))
  402          format (' Potential Grid File',10x,a)

               ifield = freeunit()
               ijlext = 3
               ijzeroes = 0 
               call numeral (ijzeroes,ext,ijlext)
               fieldfile = filename(1:leng)//'.'//ext(1:ijlext)//'e'
               call version(fieldfile,'new')
               open (unit=ifield,file=fieldfile,status='new')
               write(ifield,404) n,title(1:ltitle)
  404          format(i6,2x,a)
               do i = 1, npole
                  if (polarity(i) .ne. 0.0d0) then
                     k = ipole(i)
                     debyes = debye / polarity(i)
c                    units are debye/angstrom^3
                     if (solvtyp .eq. 'PB') then 
                        write(ifield,403) k,name(k),
     &                       (debyes*uinds(l,i),l=1,3)
                     else if (solvtyp .eq. 'PB-HPMF') then
                        write(ifield,403) k,name(k),
     &                       (debyes*uinds(l,i),l=1,3)
                     else
                        write(ifield,403) k,name(k),
     &                       (debyes*uind(l,i),l=1,3)
                     end if
  403                format (i6,2x,a3,3f12.6)   
                  end if
               end do
c               write (iout,405) fieldfile(1:trimtext(fieldfile))
c  405          format (' Field at atom site file',10x,z)
            end if
c Andrew added, END
            do i = 1, npgrid(j)
               xi = pgrid(1,i,j)
