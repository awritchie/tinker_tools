...
c Andrew Added, end
      ...
      character*7 ext
      integer i,j,k,iind,trimtext,freeunit,lext,zeroes
c
c
...
c
c     write-out induced dipoles
c
      if (use_polar) then
         iind = freeunit ()
         lext = 3
         zeroes = 0
         call numeral (zeroes,ext,lext)
  ...
  170    format (i6,2x,a)
         do i = 1, npole
            if (polarity(i) .ne. 0.0d0) then
               k = ipole(i)
               debyes = debye
c               debyes = debye / polarity(i)
c               write (iind,200) k,name(k),polarity(i)
c  200          format (i6,2x,a3,1f24.12)
               if (solvtyp .eq. 'PB') then 
                  write (iind,180)  k,name(k),(debyes*uinds(j,i),j=1,3)
               else if (solvtyp .eq. 'PB-HPMF') then
                  write (iind,180)  k,name(k),(debyes*uinds(j,i),j=1,3)
               else           
                  write (iind,180)  k,name(k),(debyes*uind(j,i),j=1,3)
                end if
  180          format (i6,2x,a3,3f24.12)
            end if
         end do
         close (unit=iind)
