...
c
c     write out field = uind / polarization
c
      if (use_polar) then
         iind = freeunit()
         lext = 3
  ...
  170    format(i6,2x,a)
         do i =1, npole
            if (polarity(i) .ne. 0.0d0) then
               k = ipole(i)
c              units of kbT/eA
               kfac = debye * 299.8391483043805 / 2.5852 / polarity(i)
               if (solvtyp .eq. 'PB') then
                  write(iind, 180) k, name(k), (kfac*uinds(j,i),j=1,3)
               else if (solvtyp .eq. 'PB-HPMF') then
                  write(iind, 180) k, name(k), (kfac*uinds(j,i),j=1,3)
               else
                  write(iind, 180) k, name(k), (kfac*uind(j,i),j=1,3)
               end if
  ...
  190    format(' Induced Dipole Field',10x,a)
      end if 
